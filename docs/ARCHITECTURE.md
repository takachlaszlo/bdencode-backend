...
PNG-n kívül JPEG/WebP/resizing nem használható. Minden fájl SHA-256 hash-t kap; ImgBB után az eredeti URL-ről letöltött byte-ok hashének is egyeznie kell. Az SSIM és PSNR a kiválasztott lossless reference/encode PNG-párokon, páronként és összesítve készül. A comparison nem futtat teljes filmes VMAF-, SSIM- vagy PSNR-menetet, így a költsége a rövid probe-ablakok és a legfeljebb öt képpár feldolgozására korlátozott. A teljes videó-comparison hard időkerete 300 másodperc; a worker ezt túllépve review állapotot kér ahelyett, hogy korlátlan elemzést folytatna. A korábbi `comparison_frames_per_type` mező csak konfigurációbetöltési kompatibilitásból marad meg, az új algoritmus nem használja.
