...
def test_comparison_pair_count_defaults_to_five() -> None:
    settings = Settings().validate()

    assert settings.comparison_pair_count == 5


@pytest.mark.parametrize("pair_count", (3, 4, 5))
...
@pytest.mark.parametrize("pair_count", (2, 6))
def test_comparison_pair_count_rejects_values_outside_bounds(pair_count: int) -> None:
    with pytest.raises(
        ConfigurationError,
        match="comparison_pair_count must be between 3 and 5",
    ):
...
def test_comparison_pair_count_environment_value_is_coerced_to_int(
    ...
    settings = load_settings(
        ...
        env={"BDENCODE_COMPARISON_PAIR_COUNT": "3"},
    )

    assert settings.comparison_pair_count == 3
